...
def main(lr=0.08, w=3, l2_reg=0.0002, batch_size=30, num_classes=2):
    ...
    model = BCNN(s=train_data.max_length, w=w, l2_reg=l2_reg, num_classes=num_classes)
    optimizer = tf.train.AdagradOptimizer(lr).minimize(model.cost)

    # For GTX 970 memory issues
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        while train_data.is_available():
            ...
            i += 1
            print("cost", i, ":", c, ", train_batch_acc:", train_acc)
...
main(lr=0.001, l2_reg=0.0002, batch_size=100)
